...
public class UpgradedImageResizer implements Runnable {
    private File[] files;
    ...
    public UpgradedImageResizer(File[] files, int newWidth, String dstFolder, long start, LogMode logMode) {
        this.files = files;
            ...
            for(File file : files)
            {
                ...
                for (int x = 0; x < newWidth; x++) {
                    for (int y = 0; y < newHeight; y++) {
                        newImage.setRGB(x, y, averageRGB(getPixelsSegment(image, x * widthStep, y * heightStep, widthStep, heightStep)));
                    }
                }
    ...
    private int averageRGB(List<Integer> rgbs) {
        int averageRGB = 0;
        int averageRed = 0;
        int averageGreen = 0;
        int averageBlue = 0;
        ...
        for (Integer rgb : rgbs) {
            ...
            averageAChannel += aChannel;
            averageRed += red;
            averageGreen += green;
            averageBlue += blue;
        }

        averageAChannel = averageAChannel / rgbs.size();
        averageRed = averageRed / rgbs.size();
        averageGreen = averageGreen / rgbs.size();
        averageBlue = averageBlue / rgbs.size();
